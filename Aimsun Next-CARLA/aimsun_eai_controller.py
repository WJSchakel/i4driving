...
#The port is the "bridge" through which data from different applications communicate (this python script with carla and this python script with aimsun)
...
#world = client.generate_opendrive_world(open("C:/Users/alkis.papadoulis/Downloads/CARLA_0.9.15/WindowsNoEditor/PythonAPI/examples/Town01.xodr", 'r').read())
world = client.load_world('Town10HD')
settings = world.get_settings()
settings.synchronous_mode = True  # Enable sync mode
settings.fixed_delta_seconds = 0.1  # 20 FPS, or pick your own step rate

#set spectator camera location
spectator = world.get_spectator()
spec_location = carla.Location(x=2, y=316, z=100)
spec_rotation = carla.Rotation(pitch=270, yaw=0, roll=0)
...
if spawn_points:
    ...
    print("First spawn point:", first_spawn_point)


def get_closest_spawn_point(world, target_location):
	spawn_points = world.get_map().get_spawn_points()

	# Use Euclidean distance (ignoring Z unless needed)
	closest = min(spawn_points, key=lambda sp: sp.location.distance(target_location))

	return closest

target_location = carla.Location(x=-199.6, y=409.3, z=2.0)
custom_spawn_point_closer = get_closest_spawn_point(world, target_location)



custom_spawn_point = carla.Transform(
    carla.Location(x=-199.6, y=409.3, z=2.0),  # Position in the world
    carla.Rotation(pitch=0.0, yaw=270.0, roll=0.0)  # Orientation
)

map = world.get_map()
waypoints = map.generate_waypoints(2.0)
vehicle_bp.set_attribute('color', '255,0,255')  # RGB: bright magenta
vehicle = world.spawn_actor(vehicle_bp, custom_spawn_point_closer)
vehicle.set_autopilot(True) #Simplistic autonomous driver in CARLA

traffic_manager = client.get_trafficmanager()
traffic_manager.set_global_distance_to_leading_vehicle(2.5)  # Optional: safer spacing
traffic_manager.vehicle_percentage_speed_difference(vehicle, 50.0)  # Reduce speed by 50%
...
def parser(aimsun_output):
	if aimsun_output.HasField('out'):
		...
		for i in aimsun_output.out.agents:
			...
			if(v == None):
				vehicles.append(Agent(i.aimsun_id, listapos, i.agent, i.speed, i.owner, spawn_vehicle(listapos[0].x, listapos[0].y, agent_type)))
			else:
...
class Agent:
	...
	def destroy_actor(self):
		if self.vehiculo_carla.is_alive:
			self.vehiculo_carla.destroy()

	# Updates the position of the object but not the rotation since it is not able to obtain it from the proto
...
if load_answer.HasField('load_result'):
	...
	close = False
	#program loop, while the connection is not closed this loop will be executed, the connection will be closed when aimsun stops giving outputs
	while not close:
		...
		aimsun_vehicle_ids = [i.aimsun_id for i in aimsun_output.out.agents]
		...
		all_ids = [vehi.id_v for vehi in vehicles]
		difference = list(set(all_ids) - set(aimsun_vehicle_ids))
		print(f"Difference is {difference}")

		for id in difference:
			car_object = next((c for c in vehicles if c.id_v == id), None)
			if car_object:
				print(car_object.id_v)
				car_object.destroy_actor()
		...
		if vehicle:
			...
			location = transform.location + carla.Location(z=60)  # 30 meters above
			rotation = carla.Rotation(yaw=180, pitch=-90)  # Looking straight down
